...
public class WeatherToHumanMapper(User user) : IMapper<OpenWeatherApiResponse, string>
{
	...
	public string Map(OpenWeatherApiResponse weather)
	{
		...
		_todaysWeather = GetWeatherForToday(new OpenWeatherHourlyDataMapper(_sunData).Map(weather.Hourly, weather.TimezoneOffset));

		var generalDaytimeWeatherCondition = GetGeneralDaytimeWeatherCondition();
		var maxTemp = GetMaxTemp();
		var minTemp = GetMinTemp();
		var willRain = WillRainDuringDaytime();
		...
		{
			var hour = data.DateTime.ToString("htt");
			var conditionEmoji = new ConditionToEmojiMapper().Map(data.Condition);
			var actualTemp = data.Temperature;
	...
	{
		var daytimeData = GetDaytimeWeather(_todaysWeather);
